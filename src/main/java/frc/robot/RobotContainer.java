...
import frc.robot.robotSettings.OIConstants;
import frc.robot.robotSettings.SwerveConstants;
...
public class RobotContainer {
  ...
  // .withDeadband(SwerveConstants.MaxSpeed *
  // 0.1).withRotationalDeadband(SwerveConstants.MaxAngularRate * 0.1) // Add
  // // a
  // // 10%
  // // deadband
  ...
  public RobotContainer() {
    sensorManager = new SensorManager(true);
    //currentSensor = sensorManager.getActive();
    ...
    // CommandXboxController(OIConstants.kOperatorController);
    configureBindings();
  ...
  public void configureTestCommands() {
    ...
    //commandsTab.add("current distance sensor", currentSensor);
    // commandsTab.add("Run Intake Wheels", new WaitCommand(10).alongWith(new
    // ShootNoteVoltage(shooter, () -> shooter.getShootVoltage())));
  }
  ...
  private void configureBindings() {
    ...
    // drive.withVelocityX(-driverController.getLeftY() * SwerveConstants.MaxSpeed)
    // // Drive forward with
    // // negative Y (forward)
    ...
    driverController.a().onTrue(new InstantCommand(() -> {
      sensorManager.setMXP();
      currentSensor = sensorManager.getActive();
    }));

    driverController.b().onTrue(new InstantCommand(() -> {
      sensorManager.setOnboard();
      currentSensor = sensorManager.getActive();
    }));
