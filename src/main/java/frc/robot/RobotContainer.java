...
public class RobotContainer {
  ...
  private final ShooterIRSensor breakBeamSensorShooter;
  private final IntakeIRSensor breakBeamSensorIntake;
  ...
  public RobotContainer() {
    ...
    shooterRotation = new ShooterRotation(arm::getArmAngle); // Shooter rotation encoder depends on arm angle,
                                                             // inject this.
    breakBeamSensorShooter = new ShooterIRSensor();
    ...
    breakBeamSensorIntake = new IntakeIRSensor();
    ...
    // Add all subsystems to their respective shuffleboard tabs.
    ...
    // This is crucial for Path Planner to work with the commands that we make.
    registerAllAutoCommands();

    // Set up all default and teleoperated commands.
  ...
  private void configureDriverController() {
    // START for RESTART -> reset the field-centric heading on left bumper press
    driverController.start()
        .onTrue(
            drivetrain.runOnce(
                drivetrain::seedFieldRelative));

    // LEFT TRIGGER for Deploy intake
  ...
  public void configureOperatorController() {
    // LEFT BUMPER for Load note into shooter
    ...
    // .onTrue(new ParallelCommandGroup(
    // new ShootAcrossField(shooterRotation, shooterWheels, servos,
    // breakBeamSensorShooter)
    // ))
    ...
    operatorController.povDown()
        .onTrue(new RotateArmToPosition(arm,
            () -> ArmConstants.ArmPIDForExternalEncoder.kArmRotationFeederSetpoint));
  }
  ...
  private void setupArmCommands() {
    arm.getShuffleboardTab().add("2 Deg Arm Rotate",
        new RotateArmToPosition(arm,
            () -> ArmConstants.ArmPIDForExternalEncoder.kArmRotationFeederSetpoint));
  }
  ...
  private void setupShooterCommands() {
    ...
    shooterTab.add("-15 Degree Rotate", new RotateShooterToPosition(shooterRotation,
        () -> ShooterConstants.RotationPIDForExternalEncoder.kArm60InchSetpoint));

    shooterTab.add("-30 Degree Rotate",
